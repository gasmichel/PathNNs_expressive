...
import argparse
import os
from preprocess_data import PathTransform


class PeptidesFunctionalDataset(InMemoryDataset):
    def __init__(
        self, root="datasets", smiles2graph=None, transform=None, pre_transform=None
    ):
        """
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--cutoff", type=int, default=5, help="Max length of shortest paths"
    )
    parser.add_argument(
        "--r",
        default=None,
        type=float,
        help="Whether to consider paths from node u to v AND paths from node v to u",
    )
    args = parser.parse_args()
    dataset = "peptides_functional"

    cutoff = args.cutoff
    for path_type in ["shortest_path", "all_shortest_paths", "all_simple_paths"]:
        print(f"Preprocessing {dataset} - {path_type}".upper())
        r = str(args.r) if args.r else "1"
        root = os.path.join(
            "datasets", dataset + "_" + path_type + "_cutoff_" + str(cutoff) + "_r_" + r
        )
